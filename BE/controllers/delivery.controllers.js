...
async function calculateFeeFromProfile(req, res) {
  const {
    profileId,
    weight = 500,
    length = 10,
  ...
  if (!profileId)
    return res.status(400).json({ success: false, message: "Thiếu profileId" });
...
async function createOrderGhnFromOrder(req, res) {
  ...
  try {
    ...
    let province =
      ...
      provinces.find(
        (p) =>
          normalizeText(p.ProvinceName) === PROVINCE_ALIASES[parsedProvinceNorm]
      );
    ...
    if (order_code) {
      ...
      // Update order status, accountId và deliveryId
      await Order.update(
        {
          status: "delivered",
          accountId: req.user?.accountId,
          deliveryId: newDeliveryId, // <-- Thêm dòng này!
        },
        { where: { orderId } }
      );
    }
...
async function getWardsByDistricts(req, res) {
...
async function getOrderDetail(req, res) {
  ...
  try {
    ...
    console.error(
      "GHN getOrderDetail error:",
      error?.response?.data || error.message
    );
    res.status(500).json({
      success: false,
      message: "Lỗi lấy chi tiết đơn hàng GHN",
      error: error?.response?.data || error.message,
    });
  }
}
...
module.exports = {
  ...
  createOrder,
  getOrderInfo,
  calculateFeeFromProfile,
  createOrderGhnFromOrder,
