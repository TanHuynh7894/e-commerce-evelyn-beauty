...
const { validateOrderDetail, logOrderDetailRequest } = require('../middlewares/orderDetail.middlewares');
const { verifyToken, requireRole } = require('../middlewares/auth');

// Tạo mới orderDetail
router.post('/', verifyToken, requireRole('CU'), logOrderDetailRequest, validateOrderDetail, orderDetailController.createOrderDetail);
// Lấy tất cả orderDetail theo orderId (bằng body)
router.post('/get-all', verifyToken, requireRole('CU'), logOrderDetailRequest, orderDetailController.getAllOrderDetails);
